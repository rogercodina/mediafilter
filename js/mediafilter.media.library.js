...
Drupal.behaviors.mediafilterMediaLibrary = {
  attach: function (context, settings) {
    var library = new Drupal.media.browser.mediafilter(Drupal.settings.media.browser.mediafilter);
    $('#media-browser-tabset').bind('tabsshow', function (event, ui) {
      if (ui.tab.hash === '#media-tab-mediafilter') {
        // Grab the parameters from the Drupal.settings object
        var params = {};
        for (var parameter in Drupal.settings.media.browser.mediafilter) {
          params[parameter] = Drupal.settings.media.browser.mediafilter[parameter];
        }
...
function mediafilter_get_filter_values(event, ui) {
  var library = new Drupal.media.browser.mediafilter(Drupal.settings.media.browser.mediafilter);
  var selected_checkboxes = $('input:checked');
  var checkboxes = {};
  var panel = $('#media-tab-mediafilter');

  selected_checkboxes.each(function( index, value ) {
    var field_name = $(value).attr('name').split('[');
    var field = field_name[0];
    if (!checkboxes[field]) {
        checkboxes[field] = {};
    }
    checkboxes[field][index] = $(value).attr('value');
  });
  var params = {};
  for (var parameter in Drupal.settings.media.browser.mediafilter) {
    params[parameter] = Drupal.settings.media.browser.mediafilter[parameter];
  }
  params['selection'] = checkboxes;
  params['selection_filename'] = $('#edit-filename').val();

  library.start($(panel), params);
  $('#scrollbox').bind('scroll', library, library.scrollUpdater);
}

Drupal.behaviors.mediafilterMediaLibraryFilter = {
  attach: function (context, settings) {

    $('#mediafilter-browser-filter-form input').bind('change', function (event, ui) {
      mediafilter_get_filter_values(event, ui);
    });

    $('#edit-filename').bind('keyup', function (event, ui) {
      mediafilter_get_filter_values(event, ui);
    });
...
Drupal.media.browser.mediafilter = function (settings) {
  this.settings = Drupal.media.browser.mediafilter.getDefaults();
...
Drupal.media.browser.mediafilter.getDefaults = function () {
  return {
...
Drupal.media.browser.mediafilter.prototype.start = function (renderElement, params) {
 ...
 */
Drupal.media.browser.mediafilter.prototype.loadMedia = function () {
  var that = this;
  if (this.params.selection && this.cursor == 0) {
    $('#media-browser-custom-library-list').empty();
  }
  $('#mediafilter-status').text(Drupal.t('Loading...')).show();
  $.extend(this.params, {start: this.cursor, limit: this.settings.limit});

  var gotMedia = function (data, status) {
    $('#mediafilter-status').text('').hide();
    ...
    if (data.media.length == 0) {
      $('#mediafilter-status').text(Drupal.t('No media found!')).show();
    }
    else {
 ...
 */
Drupal.media.browser.mediafilter.prototype.getNextMedia = function () {
...
Drupal.media.browser.mediafilter.prototype.render = function (renderElement) {
...
Drupal.media.browser.mediafilter.prototype.mediaSelected = function (media) {
  Drupal.media.browser.selectMedia(media);
};

Drupal.media.browser.mediafilter.prototype.singleSelect = function (event) {
...
Drupal.media.browser.mediafilter.prototype.multiSelect = function (event) {
  var lib = event.data.obj
  var file = event.data.file;
  event.preventDefault();
  event.stopPropagation();
